...
const UserDetailsModal = () => {
  ...
  const { isPersonalDetailsModalOpen, closePersonalDetailsModal } =
    useSettingsModal();
  ...
  if (isDesktop) {
    return (
      <Dialog
        open={isPersonalDetailsModalOpen}
        onOpenChange={closePersonalDetailsModal}
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Fill in your personal details</DialogTitle>
          </DialogHeader>
          <div className="pt-3">
            <Form {...form}>
              <form
                onSubmit={form.handleSubmit(onSubmit)}
                className="flex flex-col gap-y-4 w-full relative"
              >
                <FormField
    ...
    >
      <DrawerContent className="h-max overflow-y-scroll scrollbar-hide">
        <DrawerHeader className="text-left">
          <DrawerTitle>Add a new symptom</DrawerTitle>
        </DrawerHeader>
